var app = new Vue({
    ...
    data: {
        ...
        pageNo: 1,
        list: [],
        entry:{specification:{},optionList:[]},
        ids: [],
        searchEntity: {}
    },
    methods: {
        ...
        //该方法只要不在生命周期的
        add: function () {
            axios.post('/specification/add.shtml', this.entry).then(function (response) {
        ...
        update: function () {
            axios.post('/specification/update.shtml', this.entry).then(function (response) {
                if (response.data.success) {
                    window.location.reload()
                }
            }).catch(function (error) {
        ...
        save: function () {
            if(app.entry.specification.id != null || app.entry.specification.id !=undefined){
                this.update()
            }else {
                this.add()
            }
        ...
        addTableRow:function () {
            this.entry.optionList.push({});
        },
        removeTableRow:function (index) {
            this.entry.optionList.splice(index,1)
        }
